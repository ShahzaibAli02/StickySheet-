...
package com.agrial.loginapplication.sticksheetdetection.objectdetection;
...
import com.agrial.loginapplication.sticksheetdetection.camera.CameraReticleAnimator;
import com.agrial.loginapplication.sticksheetdetection.camera.FrameProcessorBase;
...
public class ProminentObjectProcessor extends FrameProcessorBase<List<FirebaseVisionObject>> {
  ...
  protected void onSuccess(
      ...
      GraphicOverlay graphicOverlay, int rotation) {
    ...
    if (objects.isEmpty()) {
      ...
      if (isObjectZoomEnough(graphicOverlay,object)){
        // object is big enough to capture image
        if (objectBoxOverlapsConfirmationReticle(graphicOverlay, object)) {
          ...
          workflowModel.confirmingObject(
                  new DetectedObject(object, objectIndex, image,rotation), confirmationController.getProgress());
        } else {
        ...
        // object detected but is not zoomed (big) enough
        confirmationController.reset();
        workflowModel.setWorkflowState(WorkflowModel.WorkflowState.REQUIRE_ZOOM);
      }
    }
    ...
    if (objects.isEmpty()) {
      ...
      if (isObjectZoomEnough(graphicOverlay,objects.get(0))){
        // object is big enough to capture image
        ...
        // object is not big enough to capture
        graphicOverlay.add(
                new ObjectGraphicInProminentMode(
                        graphicOverlay, objects.get(0), confirmationController));
        graphicOverlay.add(new ObjectReticleGraphic(graphicOverlay, cameraReticleAnimator));
  ...
  private boolean isObjectZoomEnough(GraphicOverlay graphicOverlay, FirebaseVisionObject object){

    RectF boxRect = graphicOverlay.translateRect(object.getBoundingBox());

    if (boxRect.width() >= 0.80*graphicOverlay.getWidth() || boxRect.height() >= 0.80*graphicOverlay.getHeight()){
      Log.d(TAG,"Object width and height: "+boxRect.width()+" ; "+boxRect.height()+" === "+"Preview width and height: "+graphicOverlay.getWidth()+" ; "+graphicOverlay.getHeight());
      Log.d(TAG,"Object size is enough");
      return true;
    }
    else {
      Log.d(TAG,"Object width and height: "+boxRect.width()+" ; "+boxRect.height()+" === "+"Preview width and height: "+graphicOverlay.getWidth()+" ; "+graphicOverlay.getHeight());
      Log.d(TAG,"object is smaller than required size");
      return false;
    }
  }
